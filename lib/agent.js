...
const SCHEMA = `{
  ...
  "api": { "tiene": boolean, "base_url": string, "auth": string, "ejemplo": string },
...
Herramienta: abrir_pagina(url) → te devuelve título, meta, texto visible y los LINKS de esa página.
...
- Cuando tengas suficiente para completar el catálogo, devolvé action:final.
...
const CANON = { 'personas desaparecidas': 'Desaparecidos', 'mapas de ayuda': 'Mapas', 'edificaciones dañadas': 'Edificaciones', 'recursos directorio': 'Directorio' }
...
export async function analizarConAgente(seedUrl, { sameDomain = [] } = {}) {
  ...
  for (let step = 0; step < MAX_STEPS; step++) {
    ...
    if (p.action === 'final') return normalize(p, title, seedUrl)
  ...
  try {
    ...
    if (p) return normalize(p, title, seedUrl)
  } catch {}
  return normalize({ riesgo: 'dudoso', motivo_riesgo: 'El agente no pudo completar el análisis.', relevante: true }, title, seedUrl)
}
