...
import { analizarNoticiaConAgente } from './news-agent'
...
export async function ingestNoticia(rawUrl) {
  ...
  // Si la extracción simple quedó POBRE (SPA/listado/vacía), escalá al AGENTE:
  // navega a la nota real y reintenta. Cae en silencio si el agente falla.
  const pobre = !p || !String(p?.resumen || '').trim() || (sc.text || '').length < 400
  if (pobre) {
    try {
      const ag = await analizarNoticiaConAgente(sc.finalUrl)
      if (ag && ag.resumen) p = ag
    } catch (e) {
      console.error('agente de noticias falló:', e?.message || e)
    }
  }
