...
import { analizarConAgente } from './agent'
...
async function analyzeUrl(rawUrl) {
  ...
  // Si la extracción simple quedó POBRE o falló, escalamos al AGENTE: navega
  // otras páginas (docs/api/github/links) y reintenta. Es el "si no trae nada,
  // el LLM evalúa y prueba otro link". Cae en silencio si el agente falla.
  const pobre =
    !a.descripcion ||
    (a.categorias?.length === 1 && a.categorias[0] === 'Otros') ||
    (a.riesgo === 'dudoso' && /no se pudo analizar/i.test(a.motivo_riesgo || ''))
  // API incierta: hay señales de API (links a docs/api/github) pero no se detectó,
  // o se declaró pero no respondió al verificar → que el agente cave y PRUEBE.
  const apiIncierta =
    (scraped.links?.length > 0 && !a.api?.tiene) ||
    (a.api?.tiene && !a.api?.verificada)
  if (pobre || apiIncierta) {
    try {
      const ag = await analizarConAgente(scraped.finalUrl, { sameDomain })
      if (ag && ag.descripcion) a = ag
    } catch (e) {
      console.error('escalado al agente falló:', e?.message || e)
    }
  }
...
export async function ingestUrl(rawUrl) {
  ...
  const a = r.analisis
  const { id, estado, duplicate } = addSitio({ ...a, finalUrl: r.finalUrl, imagen: r.imagen })
  if (duplicate) return { url: r.url, status: 'duplicado', nota: 'Ya estaba en el directorio.' }
