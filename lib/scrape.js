...
const COMMON_DOC_PATHS = ['/docs', '/api']
...
// Links GENERALES de una página (mismo dominio, con su texto de ancla). Sirve para
// que el agente de noticias navegue de un listado/portada a la nota real.
export function generalLinks(html, base) {
  const out = []
  const seen = new Set()
  let dom = ''
  try { dom = new URL(base).hostname.replace(/^www\./, '') } catch {}
  const re = /<a\b[^>]*href=["']([^"'#]+)["'][^>]*>([\s\S]*?)<\/a>/gi
  let m
  while ((m = re.exec(html)) && out.length < 25) {
    try {
      ...
      if (dom && new URL(abs).hostname.replace(/^www\./, '') !== dom) continue
      const text = m[2].replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim()
      if (text.length < 15) continue // descarta íconos/nav
      seen.add(abs)
      out.push({ url: abs, text: text.slice(0, 120) })
    } catch {}
  }
  return out
}

// Scrape de una NOTA para el agente: más texto y los links generales (para navegar).
export async function scrapeArticle(rawUrl) {
  const { finalUrl, status, html } = await safeFetch(rawUrl)
  if (status >= 400) throw new Error(`El sitio respondió ${status}`)
  const titleM = html.match(/<title[^>]*>([\s\S]*?)<\/title>/i)
  const title = titleM ? decode(titleM[1]) : ''
  ...
  return { finalUrl, title, description, text, links: generalLinks(html, finalUrl) }
}
