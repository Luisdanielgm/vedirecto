...
function requestOnce(url, pinned, { timeoutMs, maxBytes }) {
...
export async function safeFetch(rawUrl, { maxRedirects = 3, timeoutMs = 8000, maxBytes = 768_000 } = {}) {
  let url
  try {
    url = new URL(rawUrl)
  } catch {
  ...
  for (let hop = 0; hop <= maxRedirects; hop++) {
    ...
    const r = await requestOnce(url, pinned, { timeoutMs, maxBytes })
