...
export async function analizarSitio({ finalUrl, title, description, text, sameDomain = [], extras = [] }) {
  ...
  const catContext = cats.length
    ...
    : ''
  const extrasContext = extras.length
    ? `\n\nPÁGINAS ADICIONALES (docs/api/repo) — usalas para completar "endpoints", "api" y "funcionalidades":\n${extras.map((e) => `--- ${e.url} ---\n${e.text}`).join('\n\n')}`
    : ''
...
${text || '(vacío)'}${dupContext}${catContext}${extrasContext}`
  ...
  // Sinónimos conocidos → canónico (evita que vuelva a fragmentar).
  const CANON = {
