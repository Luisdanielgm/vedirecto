...
import { verificarApi } from './api-probe.js'
...
Te paso la URL y el contenido scrapeado de un sitio. Devolvé SOLO un objeto JSON con estos campos:
{
  ...
  "api": {                             // si expone una API pública (muchos son open source con API abierta)
    ...
    "potencial": boolean,              // si NO tiene API ahora pero PODRÍA sumarla: open source/GitHub, datos abiertos o descargables, o menciona API futura/roadmap
    "potencial_motivo": string         // por qué tiene potencial, o ""
  },
...
${CLOSE}${dupContext}${catContext}${reindexBlock}`
  ...
  const api = (() => {
    ...
    return {
      ...
      potencial: a.potencial === true,
      potencial_motivo: String(a.potencial_motivo || '').trim(),
    }
  })()
  // Verificación REAL: si dice tener API, la sondeamos una vez (GET) para saber si responde.
  const ver = await verificarApi(api)
  api.verificada = ver.verificada
  api.verificada_status = ver.verificada_status
  return {
