...
export function registrarVisita(tipo, sitioId = null) {
  const dia = new Date().toISOString().slice(0, 10)
  getDb().prepare('INSERT INTO visitas (tipo, sitio_id, dia) VALUES (?, ?, ?)').run(tipo, sitioId, dia)
}
...
export function estadisticas() {
  ...
  const porDia = db
    .prepare("SELECT dia, COUNT(*) AS n FROM visitas WHERE tipo = 'pagina' GROUP BY dia ORDER BY dia DESC LIMIT 14")
    .all()
  const topSitios = db
    .prepare(
    ...
    .all()
  return { total, totalClicks, porDia, topSitios }
}
